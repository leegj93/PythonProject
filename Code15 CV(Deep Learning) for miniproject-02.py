...
def leftMouseMove(event):
    ...
    # boxLine = canvas.create_line(ex, ey, event.x, event.y)
    # ex = event.x;
    # ey = event.y
    global x0, y0, ex, ey
    canvas.create_line(x0, y0, event.x, event.y)
    x0, y0 = event.x, event.y
    MM=[[0 for i in range(inH)]for k in range(inW)]
    MM = [[x0],[y0]]
    print(MM)
        ...
        for i in range(inH) :
            for k in range(inW) :
                ...
                if sx <= k <= ex and sy <= i <= ey:
                    ...
                    # if inImage[R][i][k] + value and inImage[G][i][k]+ value and inImage[B][i][k]+ value > 255:
                    #     outImage[R][i][k], outImage[G][i][k], outImage[B][i][k] = 255, 255, 255
                    # elif inImage[R][i][k] + value and inImage[G][i][k]+ value and inImage[B][i][k]+ value  < 0:
                    #     outImage[R][i][k], outImage[G][i][k], outImage[B][i][k] = 0, 0, 0
                    # else:
                    #     outImage[R][i][k], outImage[G][i][k], outImage[B][i][k] = inImage[R][i][k] + value, inImage[G][i][k]+ value, \
                    #                                                           inImage[B][i][k]+ value
                else:
...
def revImageColor(): ## 완료
    ...
    maxValMM = minValMM =[[[0] for i in range(inH)]for k in range(inW)]

    for i in range(inH):
        for k in range(inW):
            if  MM[i][k] < minValMM[i][k]:
                minValMM[i][k] = MM[i][k]
            elif MM[i][k] > maxValMM[i][k]:
                maxValMM[i][k] = MM[i][k]
    ...
    for RGB in range(3):
        for i in range(inH):
            for k in range(inW):
                ...
                if minValMM[i][k] <= k <= maxValMM[i][k] and minValMM[i][k] <= i <= maxValMM[i][k]:
                    # h, s, v = inImageHSV[0][i][k], inImageHSV[1][i][k], tmpOutImageV[i][k]
                    # r, g, b = colorsys.hsv_to_rgb(h, s, v)
