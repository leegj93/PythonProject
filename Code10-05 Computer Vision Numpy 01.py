...
def  bwImage() :
    ...
    outImage = np.where(outImage > avg, 255, outImage)
    outImage = np.where(outImage < avg, 0, outImage)
    # for i in range(inH) :
    #     for k in range(inW) :
    #         if inImage[i][k] > avg :
    #             outImage[i][k] = 255
...
def  paraImage() :
    ...
    LUT = (255-255*(np.arrange(256)/128-1)*(np.arrange(256)/128-1))
...
def mouseDrop(event) :
    ...
    # outImage = np.where(my >= 0 & my < outW & mx >= 0 & mx < outH, inImage[my][mx], 0)
