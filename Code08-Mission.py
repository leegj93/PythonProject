...
def downloadData():
    ...
    cur = con.cursor()
    value = askinteger("사진번호", "불러올 사진==>", minvalue=1)
    sql = "SELECT raw_fname, raw_data FROM rawImage_TBL WHERE raw_id =" + str(value)
    cur.execute(sql)
...
def morphImage():
    ...
    inH2 = inW2 = int(math.sqrt(fsize))  # 핵심 코드 -> 정사각형이므로 루트를 씌우면 가로와 높이가 나옴.

    ## 입력영상 메모리 확보 ## - 512 x 512를 0으로 초기화 시킴.

    inImage2 = []  # load가 계속 될 수 있기 때문에 초기화 시키는 용도로 사용.
    inImage2 = malloc(inH2,inW2)
    with open(filename2, 'rb') as rfp:  # 우리는 binary 이므로 rb 사용.(이미지이기 때문에 binary) cf. txt는 r
        for i in range(inH2):
            for k in range(inW2):
                inImage2[i][k] = int(ord(rfp.read(1)))  # 1바이트만 읽는다.
    ###################################
    outImage = []
    outImage = malloc(outH, outW)
    ########진짜 컴퓨터 비전 알고리즘 ########
...
mainMenu.add_cascade(label="SQL", menu=comVisionMenu5)
comVisionMenu5.add_command(label="불러오기", command=downloadData)
# comVisionMenu5.add_command(label="불러오기", command=embossImage)
