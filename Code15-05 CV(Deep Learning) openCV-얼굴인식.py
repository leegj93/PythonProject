...
def loadImageColor(fnameOrCvData):
    ...
    if type(fnameOrCvData) == str:
        cvData = cv2.imread(fnameOrCvData)
    else:
...
def rotateOpenCv():
    ...
    cvPhoto2 = cvPhoto[:]
    rotate_matrix = cv2.getRotationMatrix2D((outW//2, outH//2), angle, 1)## 중앙점, 각도, 스케일
...
def zoomInOpenCv():
    ...
    cvPhoto2 = cvPhoto[:]
    rotate_matrix = cv2.resize(cvPhoto2,None, fx= scale, fy =scale)  ## 중앙점, 각도, 스케일
...
def waveHorOpenCv():
    ...
    cvPhoto2= np.zero(cvPhoto.shape, dtype =cvPhoto.dtype)
    for i in range(inH):
        for k in range(inW):
...
def cartoonOpenCv():
    ...
    cvPhoto2 = cv2.cvtColor(cvPhoto2, cv2.COLOR_GRAY2RGB)
    photo2 = Image.fromarray(cvPhoto2)
    #############################################################
    toColorOutArray(photo2)
...
def hanibalOpenCv():
    ...
    ####################### OpenCV 처리 부분####################
    face_cascade = cv2.CascadeClassifier("haarcascade_frontalface_alt.xml")
    faceMask = cv2.imread("C:\PySource\Project1\Images\images(ML)/mask_hannibal.png")
    cvPhoto2 = cvPhoto[:]
...
def sunglassOpenCv():
    ...
    faceMask = cv2.imread("C:\PySource\Project1\Images\images(ML)/sunglass-1.png")
    cvPhoto2 = cvPhoto[:]
    h_mask, w_mask = faceMask.shape[:2]
    grey = cv2.cvtColor(cvPhoto2, cv2.COLOR_RGB2GRAY)
    face_rects = face_cascade.detectMultiScale(grey, 1.1, 5)
...
openCVMenu.add_command(label="엠보싱 처리", command = embossOpenCv)
openCVMenu.add_command(label="그레이 스케일", command = grayscaleOpenCv)
openCVMenu.add_command(label="블러링 스케일", command = blurOpenCv)
comVisionMenu4.add_separator()
openCVMenu.add_command(label="회전", command = rotateOpenCv)
openCVMenu.add_command(label="줌인", command = zoomInOpenCv)
comVisionMenu4.add_separator()
...
openCVMenu.add_command(label="선그라스(머신러닝)", command = sunglassOpenCv)
